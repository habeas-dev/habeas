...
// docs/source-rank.js is GENERATED from this file (docs/tools/build-source-rank.mjs) — a test fails if the
// two drift, because a visitor convinced by the site and then shown an alphabetical wall of Spanish
// supermarkets inside the extension is the same failure twice.
//
...
// Lower is shown first. A source with no declared country is global in practice, so it is treated as such
// rather than sunk to the bottom as "unknown".
// Exported because a caller with its OWN ordering (the marketplace sorts by install state first) needs the
// tier to break its ties — sorting by position in the ranked list instead would silently impose catalogue
// order and kill the alphabetical fallback.
export function regionTier(source, region) {
...
export function rankSources(sources, region) {
  ...
  return list
    .map((source, i) => ({ source, i, tier: regionTier(source, where) }))
    .sort((a, b) => (a.tier - b.tier) || (a.i - b.i))
    .map((x) => x.source);
}


/** `{ language, timeZone }` for the current browser — the only two things this module reads. */
