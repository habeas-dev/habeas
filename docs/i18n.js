...
const I18N = {
  en: {
    ...
    trust_h2: 'The warning you may see',
    trust_body: 'Chrome will show a notice that the extension is not trusted. It is automatic for every new developer, it takes a few months to fade, and it says nothing about the code — which you can read in full.',
    trust_irony: 'There is an irony worth stating plainly, once: an extension built on the idea that your data should not depend on an intermediary’s goodwill is handed a mark of distrust by an intermediary that explains neither its criteria nor how to appeal them. The code is open; the judgement is yours.',
    trust_link: 'Read the code on GitHub →',
  ...
  es: {
    ...
    trust_h2: 'El aviso que quizá vea',
    trust_body: 'Chrome mostrará un aviso de que la extensión no es de confianza. Es automático para todo desarrollador nuevo, tarda unos meses en desaparecer, y no dice nada sobre el código — que puede usted leer entero.',
    trust_irony: 'Hay una ironía que conviene dejar dicha una vez, con calma: una extensión cuya tesis es que sus datos no deben depender de la buena voluntad de un intermediario recibe un sello de desconfianza de un intermediario que ni explica sus criterios ni admite recurso. El código está abierto; el juicio es suyo.',
    trust_link: 'Lea el código en GitHub →',
    nav_github: 'GitHub',
