...
- **WiZink no longer collects pending charges, which is what was saving each purchase twice.** A pending
  charge is provisional — amount, date and wording can all still change before it settles — and WiZink
  lists it again inside the statement that bills it. Both sightings were being saved. A movement is now
...
- **The copy now asks what a document has before going to look for it**, and settles the ones without a
  ...
  file of its own. A source whose documents all lack files now resolves with no loop and no requests, and
  says so; the progress counter counts what will actually be copied rather than how many entries exist. Habeas already records each
  document's formats — at delivery time, and again on the Archive's format scan — and the Archive, the
  popup and the viewer all read it. The copy did not: it rediscovered by request what was already written
  down, one round-trip per document per candidate format, largely to be told that a bank movement has no
...
- **The Archive's “Refresh” now shows progress.** It reported page-by-page for paginated sources but sat silent for
  ...
  long refresh visibly advances instead of jumping straight from start to finish.
...
- **Settings home shows destinations + explains archive vs. destinations** (`ui/options.html`, `ui/options.js`).
  ...
  this copy that wrongly implied the files themselves live in the archive. The two cards are styled as an
  informational callout (ℹ️ eyebrow, dashed/accented tint) so they read as an explainer, not as another of the
  four actionable overview cards.
...
- **External `collect` gains archive-only sync** (`background.js`, `{ fromStore: true }`): deliver the NEW
  ...
  files come along only if a site tab is already open (`sendStoredDocs` `noOpen`). Awaited: replies
  `status:'done'` with `sent`/`found` counts (found 0 = nothing new in the archive) or `status:'error'`, so the
  consumer can surface the outcome. Lets a consumer offer "sync from archive" next to "go to the source".
...
- **Source tab is surfaced ONLY when a user-initiated run FAILS on auth** (`lib/pagefetch.js`, `background.js`).
  The 401→foreground behavior no longer fires on every 401 (that jumped Chrome to a tab during background
  syncs, and even on a transient token-race 401 that recovers). Now the tab comes to the front only when a
  MANUAL save / external collect ends in a hard 401/403 failure — i.e. the session really expired and needs a
  re-login.
